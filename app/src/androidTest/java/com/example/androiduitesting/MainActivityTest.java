...
public class MainActivityTest {
    ...
    public void testActivitySwitch(){
        ...
        // Then click on the city in the list
        onData(anything()).inAdapterView(withId(R.id.city_list)).atPosition(0).perform(click());

        // After, check that ShowActivity is displayed by verifying the BACK button exists
    ...
    public void testCityNameConsistent(){
        ...
        // Then click on the city in the list to open ShowActivity
        onData(anything()).inAdapterView(withId(R.id.city_list)).atPosition(0).perform(click());

        // After, check that the city name displayed in ShowActivity matches what we entered
    ...
    public void testBackButton(){
        ...
        // Then, click on the city to go to ShowActivity
        onData(anything()).inAdapterView(withId(R.id.city_list)).atPosition(0).perform(click());

        // Press the 'back' button
        onView(withId(R.id.button_back)).perform(click());

        // Verify we are back on MainActivity by checking the 'Add City' button is visible
